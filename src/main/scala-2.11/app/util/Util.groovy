...
import app.util.image.ImageFormatter
...
public class Util {
    ...
    public synchronized static List<FileImage> getImagesFrom(Path _path, int _nrOfImages) {
        def cat = new ArrayList<String>()
        cat.add("cifra")
        return getImagesFrom(_path, cat, _nrOfImages, 0)
    }
    ...
    public synchronized static FileImage prepareFileImage(Image image, int ungle) {
        ...
        fileImage.with {
            ...
            it.semnificativeScaledImage = imageAction.scalingImage(BussinesConstants.WIDTH, BussinesConstants.HEIGHT).image;
